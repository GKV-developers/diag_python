...
def fluidtotaltrans_loop(it, iss, imom, xr_phi, xr_Al, xr_mom, flag=None, outdir="../data_fluid/"):
    ...
    from diag_geom import nxw, nyw, ns, ui, rootg, kx, ky, nx, global_ny, global_nz  # 格子点情報、時期座標情報、座標情報を呼び込む
    ...
    ikxf = ui * kx * mom  #mom[0:nmom, :, :, :] Dim (imom, zz, ky, kx)、[:]を廃止して表記方法を合理化。
    ikyf = ui * ky * mom  #mom[0:nmom, :, :, :]
    ikxp = ui * kx * phi # Dim (zz, ky, kx)、[:]を廃止して表記方法を合理化。
    ikyp = ui * ky * phi
    ...
    # imom = 0
    dfdx_0 = fft_backward_xyz(ikxf[0], nxw=nxw, nyw=nyw)
    dfdy_0 = fft_backward_xyz(ikyf[0], nxw=nxw, nyw=nyw)

    # imom = 1    
    dfdx_1 = fft_backward_xyz(ikxf[1], nxw=nxw, nyw=nyw)
    dfdy_1 = fft_backward_xyz(ikyf[1], nxw=nxw, nyw=nyw)    

    # imom = 2    
    dfdx_2 = fft_backward_xyz(ikxf[2], nxw=nxw, nyw=nyw)
    dfdy_2 = fft_backward_xyz(ikyf[2], nxw=nxw, nyw=nyw)

    # imom = 3
    dfdx_3 = fft_backward_xyz(ikxf[3], nxw=nxw, nyw=nyw)
    dfdy_3 = fft_backward_xyz(ikyf[3], nxw=nxw, nyw=nyw)

    # imom = 4
    dfdx_4 = fft_backward_xyz(ikxf[4], nxw=nxw, nyw=nyw)
    dfdy_4 = fft_backward_xyz(ikyf[4], nxw=nxw, nyw=nyw)

    # imom = 5
    dfdx_5 = fft_backward_xyz(ikxf[5], nxw=nxw, nyw=nyw)
    dfdy_5 = fft_backward_xyz(ikyf[5], nxw=nxw, nyw=nyw)    
    
    dpdx_phi = fft_backward_xyz(ikxp, nxw=nxw, nyw=nyw)
    dpdy_phi = fft_backward_xyz(ikyp, nxw=nxw, nyw=nyw)
    ...
    nf_es[0] = fft_forward_xyz(wkxy_es[0], nxw=nxw, nyw=nyw) 
    nf_es[1] = fft_forward_xyz(wkxy_es[1], nxw=nxw, nyw=nyw) 
    nf_es[2] = fft_forward_xyz(wkxy_es[2], nxw=nxw, nyw=nyw)
    nf_es[3] = fft_forward_xyz(wkxy_es[3], nxw=nxw, nyw=nyw)
    nf_es[4] = fft_forward_xyz(wkxy_es[4], nxw=nxw, nyw=nyw) 
    nf_es[5] = fft_forward_xyz(wkxy_es[5], nxw=nxw, nyw=nyw)
    ...
    dpdx_Al = fft_backward_xyz(ikxp, nxw=nxw, nyw=nyw) # for Al
    dpdy_Al = fft_backward_xyz(ikyp, nxw=nxw, nyw=nyw) # for Al
    
    # *************** tk_em の計算 *******************
    # imom = 0
    wkxy_em[0] = -dpdx_Al * dfdy_0 + dpdy_Al * dfdx_0  # 3次元実数配列, for Al
    nf_em[0] = fft_forward_xyz(wkxy_em[0], nxw=nxw, nyw=nyw)  # for Al
    ...
    nf_em[1] = fft_forward_xyz(wkxy_em[1], nxw=nxw, nyw=nyw)  # for Al
    tk_em[0] = tk_em[0] + fct * ((fcs[iss] * tau[iss] / Znum[iss]) * (-np.sqrt(tau[iss]/Anum[iss]))
    ...
    nf_em[2] = fft_forward_xyz(wkxy_em[2], nxw=nxw, nyw=nyw)  # for Al
    tk_em[1] = tk_em[1] + fct * ((fcs[iss] * tau[iss] / Znum[iss]) * (-np.sqrt(tau[iss]/Anum[iss]))
    ...
    nf_em[5] = fft_forward_xyz(wkxy_em[5], nxw=nxw, nyw=nyw)  # for Al
    tk_em[3] = tk_em[3] + fct * ((fcs[iss] * tau[iss] / Znum[iss]) * (-np.sqrt(tau[iss]/Anum[iss]))
