...
def fluxinvm_fxv(it, iss, rankz, xr_phi, xr_fxv, flag=None, outdir="../data/" ):
    ...
    if flag == 'display' or flag == 'savefig' :
        ...
        filename = os.path.join(outdir,'fluxinvm_rankz{:04d}s{:d}_t{:08d}.txt'.format(rankz, iss, it))
...
def fluxinvm_cnt(it, iss, iz, xr_phi, xr_cnt, flag=None, outdir="../data/" ):
    ...
    if flag == 'display' or flag == 'savefig' :
        ...
        filename = os.path.join(outdir,'fluxinvm_z{:04d}s{:d}_t{:08d}.txt'.format(iz, iss, it))
        with open(filename, 'w') as outfile:
            outfile.write('# iz = {:d}, zz = {:f}\n'.format(iz, zz_cnt))
            outfile.write('# it = {:d}, t = {:f}\n'.format(it, time_cnt))
            outfile.write('### Infomation of phi ###')
...
if (__name__ == '__main__'):
    ...
    from diag_geom import nprocz, global_nz

    s_time = time.time()
    
    it = 30; iss = 0; rankz = int(nprocz / 2)
    fluxinvm_fxv(it, iss, rankz, xr_phi, xr_fxv, flag="display", outdir="../data/")
    ...
    s_time = time.time()
    it = 0; iss = 0; iz = global_nz
    fluxinvm_cnt(it, iss, iz, xr_phi, xr_cnt, flag="display", outdir="../data/")
