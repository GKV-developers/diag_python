...
def fluidsubsptrans_loop(it, iss, imom, nfil, xr_phi, xr_Al, xr_mom, flag=None, outdir="../data_fluid/"):
    ...
    for imom in range(nmom): # nmom
        for jfil in range(nfil): # nfil
            for ifil in range(nfil): # nfil
                wkxy_es[:,:,:] = 0.5*(-dpdx_phi[ifil,:,:,:] * dfdy5D[imom,jfil,:,:,:]                                 + dpdy_phi[ifil,:,:,:] * dfdx5D[imom,jfil,:,:,:]                                  - dpdx_phi[jfil,:,:,:] * dfdy5D[imom,ifil,:,:,:]                                  + dpdy_phi[jfil,:,:,:] * dfdx5D[imom,ifil,:,:,:]) # wkxyは、imom, nfilの軸を持つ必要なし。 3次元配列の定義で良い。
                ...
                for kfil in range(nfil): # nfil
                    ...
                    subtrans_es[imom,kfil,jfil,ifil] =                        np.sum(fct[:,:,:]*(2.0*temp_filter[:,:,:]*(fcs[iss]*tau[iss]/Znum[iss])                                            * coeffiient[imom]                                               *(np.conj(mom[imom,:,:,:])*nf_es[:,:,:]).real))
    ...
    for jfil in range(nfil):
        for ifil in range(nfil):
            wkxy_em[:,:,:] =                 0.5*( -dpdx_Al[ifil,:,:,:] * dfdy5D[0,jfil,:,:,:]                      + dpdy_Al[ifil,:,:,:] * dfdx5D[0,jfil,:,:,:]                          - dpdx_Al[jfil,:,:,:] * dfdy5D[0,ifil,:,:,:]                              + dpdy_Al[jfil,:,:,:] * dfdx5D[0,ifil,:,:,:] )
            ...
            for kfil in range(nfil):
                temp_filter = np.array(py_filter4D[kfil,:,:,:])  # temp_filter: 3次元配列
                temp_filter[:,0,0:nx+1] = 0.0 # Fortran: do my=0 do mx=1,nx の部分を表現 
                subtrans_em[0,kfil,jfil,ifil] =                        np.sum(fct[:,:,:]*(2.0*temp_filter[:,:,:]*(fcs[iss]*tau[iss]/Znum[iss])                                            *(- np.sqrt(tau[iss]/Anum[iss]))                                               *(np.conj(mom[1,:,:,:])*nf_em[:,:,:]).real))                

            wkxy_em[:,:,:] =                 0.5*( -dpdx_Al[ifil] * dfdy5D[1,jfil,:,:,:]                      + dpdy_Al[ifil,:,:,:] * dfdx5D[1,jfil,:,:,:]                          - dpdx_Al[jfil,:,:,:] * dfdy5D[1,ifil,:,:,:]                              + dpdy_Al[jfil,:,:,:] * dfdx5D[1,ifil,:,:,:]  ) 
            ...
            for kfil in range(nfil):
                temp_filter = np.array(py_filter4D[kfil,:,:,:])  # temp_filter: 3次元配列
                temp_filter[:,0,0:nx+1] = 0.0 # Fortran: do my=0 do mx=1,nx の部分を表現 
                subtrans_em[0, kfil,jfil,ifil] = subtrans_em[0, kfil,jfil,ifil]+                          np.sum(fct[:,:,:]*(2.0*temp_filter[:,:,:]*(fcs[iss]*tau[iss]/Znum[iss])                                            *(- np.sqrt(tau[iss]/Anum[iss]))                                               *(np.conj(mom[0,:,:,:])*nf_em[:,:,:]).real)) # 追加： subtrans_em[0, kfil,jfil,ifil] +
                    # axisを指定しないので、iz、my、mx軸の全要素の合計となる。
    ...
    for jfil in range(nfil): # nfil
        for ifil in range(nfil): # nfil
            ...
            for kfil in range(nfil):
                temp_filter = np.array(py_filter4D[kfil,:,:,:])  # temp_filter: 3次元配列
                temp_filter[:,0,0:nx+1] = 0.0 # Fortran: do my=0 do mx=1,nx の部分を表現 
                subtrans_em[1, kfil,jfil,ifil] =                        np.sum(fct[:,:,:]*(2.0*temp_filter[:,:,:]*(fcs[iss]*tau[iss]/Znum[iss])                                            *(- np.sqrt(tau[iss]/Anum[iss]))                                               *(np.conj(mom[2,:,:,:])*nf_em[:,:,:]).real))
                    # axisを指定しないので、iz、my、mx軸の全要素の合計となる。

            wkxy_em[:,:,:] =                 0.5*( -dpdx_Al[ifil,:,:,:] * dfdy5D[2,jfil,:,:,:]                      + dpdy_Al[ifil,:,:,:] * dfdx5D[2,jfil,:,:,:]                          - dpdx_Al[jfil,:,:,:] * dfdy5D[2,ifil,:,:,:]                              + dpdy_Al[jfil,:,:,:] * dfdx5D[2,ifil,:,:,:] ) 
            ...
            for kfil in range(nfil):
                temp_filter = np.array(py_filter4D[kfil,:,:,:])  # temp_filter: 3次元配列
                temp_filter[:,0,0:nx+1] = 0.0 # Fortran: do my=0 do mx=1,nx の部分を表現 
                subtrans_em[1, kfil,jfil,ifil] = subtrans_em[1, kfil,jfil,ifil] +                         np.sum(fct[:,:,:]*(2.0*temp_filter[:,:,:]*(fcs[iss]*tau[iss]/Znum[iss])                                            *(- np.sqrt(tau[iss]/Anum[iss]))                                               *(np.conj(mom[1,:,:,:])*nf_em[:,:,:]).real)) # 追加： subtrans_em[1, kfil,jfil,ifil] +
                    # axisを指定しないので、iz、my、mx軸の全要素の合計となる。
    ...
    for jfil in range(nfil):
        for ifil in range(nfil):  # imom=1で作成した計算式を流用。
            ...
            for kfil in range(nfil):
                temp_filter = np.array(py_filter4D[kfil,:,:,:])  # temp_filter: 3次元配列
                temp_filter[:,0,0:nx+1] = 0.0 # Fortran: do my=0 do mx=1,nx の部分を表現 
                subtrans_em[2, kfil,jfil,ifil] =                        np.sum(fct[:,:,:]*(2.0*temp_filter[:,:,:]*(fcs[iss]*tau[iss]/Znum[iss])                                            *(- np.sqrt(tau[iss]/Anum[iss]))                                                *0.5*(np.conj(mom[4,:,:,:])*nf_em[:,:,:]).real))
                    # axisを指定しないので、iz、my、mx軸の全要素の合計となる。  

            wkxy_em[:,:,:] =                 0.5*( -dpdx_Al[ifil,:,:,:] * dfdy5D[4,jfil,:,:,:]                      + dpdy_Al[ifil,:,:,:] * dfdx5D[4,jfil,:,:,:]                          - dpdx_Al[jfil,:,:,:] * dfdy5D[4,ifil,:,:,:]                              + dpdy_Al[jfil,:,:,:] * dfdx5D[4,ifil,:,:,:] )  
            ...
            for kfil in range(nfil):
                temp_filter = np.array(py_filter4D[kfil,:,:,:])  # temp_filter: 3次元配列
                temp_filter[:,0,0:nx+1] = 0.0 # Fortran: do my=0 do mx=1,nx の部分を表現 
                subtrans_em[2, kfil,jfil,ifil] = subtrans_em[2, kfil,jfil,ifil] +                         np.sum(fct[:,:,:]*(2.0*temp_filter[:,:,:]*(fcs[iss]*tau[iss]/Znum[iss])                                            *(- np.sqrt(tau[iss]/Anum[iss]))                                                *0.5*(np.conj(mom[2,:,:,:])*nf_em[:,:,:]).real)) # 追加： subtrans_em[2, kfil,jfil,ifil] +
                    # axisを指定しないので、iz、my、mx軸の全要素の合計となる。  
    ...
    for jfil in range(nfil):
        for ifil in range(nfil):
            wkxy_em[:,:,:] =                 0.5*( -dpdx_Al[ifil,:,:,:] * dfdy5D[3,jfil,:,:,:]                      + dpdy_Al[ifil,:,:,:] * dfdx5D[3,jfil,:,:,:]                          - dpdx_Al[jfil,:,:,:] * dfdy5D[3,ifil,:,:,:]                              + dpdy_Al[jfil,:,:,:] * dfdx5D[3,ifil,:,:,:] )
            ...
            for kfil in range(nfil):
                temp_filter = np.array(py_filter4D[kfil,:,:,:])  # temp_filter: 3次元配列
                temp_filter[:,0,0:nx+1] = 0.0 # Fortran: do my=0 do mx=1,nx の部分を表現 
                subtrans_em[3, kfil,jfil,ifil] =                        np.sum(fct[:,:,:]*(2.0*temp_filter[:,:,:]*(fcs[iss]*tau[iss]/Znum[iss])                                            *(- np.sqrt(tau[iss]/Anum[iss]))                                                *(np.conj(mom[5,:,:,:])*nf_em[:,:,:]).real))
                    # axisを指定しないので、iz、my、mx軸の全要素の合計となる。  

            wkxy_em[:,:,:] =                 0.5*( -dpdx_Al[ifil,:,:,:] * dfdy5D[5,jfil,:,:,:]                      + dpdy_Al[ifil,:,:,:] * dfdx5D[5,jfil,:,:,:]                          - dpdx_Al[jfil,:,:,:] * dfdy5D[5,ifil,:,:,:]                              + dpdy_Al[jfil,:,:,:] * dfdx5D[5,ifil,:,:,:]  ) 
            ...
            for kfil in range(nfil):
                temp_filter = np.array(py_filter4D[kfil,:,:,:])  # temp_filter: 3次元配列
                temp_filter[:,0,0:nx+1] = 0.0 # Fortran: do my=0 do mx=1,nx の部分を表現 
                subtrans_em[3, kfil,jfil,ifil] = subtrans_em[3, kfil,jfil,ifil] +                         np.sum(fct[:,:,:]*(2.0*temp_filter[:,:,:]*(fcs[iss]*tau[iss]/Znum[iss])                                            *(- np.sqrt(tau[iss]/Anum[iss]))                                                *(np.conj(mom[3,:,:,:])*nf_em[:,:,:]).real)) # 追加： subtrans_em[3, kfil,jfil,ifil] + 
                    # axisを指定しないので、iz、my、mx軸の全要素の合計となる。  
